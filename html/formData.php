	...
	<?php
		...
		if(isset($_POST['submit'])){
			...
			if($available_rooms < $roomCount)
			{
				...
				$message = "Sorry, there are only " .abs($available_rooms). " $room rooms available from $check_inDate to $check_outDate. Please select a different date range or choose a different room type.";
				...
				echo "<script>alert('Your booking was successful!!! Thank You for connecting with us...')</script>";
			}
			
			mysqli_close($conn);
		}
		
		// header("Refresh: 1; url=file:///C:/xampp/htdocs/WebAmaya-main/home.html");
		header("Refresh: 1; url=/WebAmaya-main/home.html");
